...
def todos_alunos():
    with engine.connect() as con:   
        ...
        while True:
            ...
            if result == None:
                break
            d_results = dict(result)
            resultados.append(d_results) 
        return resultados
...
def add_livro(id_livro, descricao):
    with engine.connect() as con:
        sql_insert = text ("INSERT INTO livros (id_livro, descricao) VALUES (:id_livro, :descricao)")
...
def livro_parado():
    with engine.connect() as con:
        sql_consulta = text ("SELECT * FROM livros WHERE id_aluno = ISNULL")
        rs = con.execute(sql_consulta)
        resultados = []
        while True:
            result = rs.fetchone()
...
def livros_do_aluno(nome_aluno):
    with engine.connect() as con:
        sql_consulta = text ("SELECT id_livro, id_aluno, descricao FROM livro JOIN aluno ON livro.id_livro = aluno.id WHERE nome = :nome_aluno")
        rs = con.execute (sql_consulta, nome_aluno=nome_aluno)
        resultados = []
        while True:
            result = rs.fetchone()
            if result == None:
